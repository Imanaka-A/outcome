...
for i in tqdm(range(file_num, int(file_num + 1))):
    ...
    for j in range(1,11):
        ...
        Lgas = df_EMG["L.Lat. Gastro(uV):"]
        Lgas = Lgas.sort_values(ascending=False)
        Lgas = Lgas.head(j)

    Lgas_mean = Lgas.mean()
    df_EMG["L.Lat. Gastro(uV):"]=df_EMG["L.Lat. Gastro(uV):"] *100 / Lgas_mean

    for j in range(1,11):

        Rgas = df_EMG["R.Lat. Gastro(uV):"]
        Rgas = Rgas.sort_values(ascending=False)
        Rgas = Rgas.head(j)

    Rgas_mean = Rgas.mean()
    df_EMG["R.Lat. Gastro(uV):"]=df_EMG["R.Lat. Gastro(uV):"] *100 / Rgas_mean

    for j in range(1,11):

        Lrec = df_EMG["L.Rectus Fem.(uV):"]
        Lrec = Lrec.sort_values(ascending=False)
        Lrec = Lrec.head(j)

    Lrec_mean = Lrec.mean()
    df_EMG["L.Rectus Fem.(uV):"]=df_EMG["L.Rectus Fem.(uV):"] *100 / Lrec_mean

    for j in range(1,11):

        Rrec = df_EMG["R.Rectus Fem.(uV):"]
        Rrec = Rrec.sort_values(ascending=False)
        Rrec = Rrec.head(j)

    Rrec_mean = Rrec.mean()
    df_EMG["R.Rectus Fem.(uV):"]=df_EMG["R.Rectus Fem.(uV):"] *100 / Rrec_mean

    for j in range(1,11):

        LVlo = df_EMG["L.Vlo(uV):"]
        LVlo = LVlo.sort_values(ascending=False)
        LVlo = LVlo.head(j)

    LVlo_mean = LVlo.mean()
    df_EMG["L.Vlo(uV):"]=df_EMG["L.Vlo(uV):"] *100 / LVlo_mean

    for j in range(1,11):

        RVlo = df_EMG["R.Vlo(uV):"]
        RVlo = RVlo.sort_values(ascending=False)
        RVlo = RVlo.head(j)

    RVlo_mean = RVlo.mean()
    df_EMG["R.Vlo(uV):"]=df_EMG["R.Vlo(uV):"] *100 / RVlo_mean

    for j in range(1,11):

        LTib = df_EMG["L.Tib.Ant.(uV):"]
        LTib = LTib.sort_values(ascending=False)
        LTib = LTib.head(j)

    LTib_mean = LTib.mean()
    df_EMG["L.Tib.Ant.(uV):"]=df_EMG["L.Tib.Ant.(uV):"] *100 / LTib_mean

    for j in range(1,11):

        RTib = df_EMG["R.Tib.Ant.(uV):"]
        RTib = RTib.sort_values(ascending=False)
        RTib = RTib.head(j)

    RTib_mean = RTib.mean()
    df_EMG["R.Tib.Ant.(uV):"]=df_EMG["R.Tib.Ant.(uV):"] *100 / RTib_mean
    """
    ...
    for j in range(int(end_stand*GRF_SAMPLING_RATE / OPTITRACK_SAMPLING_RATE), df_grf.Time.count()):
        if df_grf.R_Sum[j] <= GRF_FORCE:
            ...
            right_grf_0 = j
            right_grf_0_time = j / GRF_SAMPLING_RATE
            break
